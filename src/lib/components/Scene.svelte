<script lang="ts">
	...
	import { OrbitControls, useCursor, type IntersectionEvent, interactivity } from '@threlte/extras';
  ...
  `;
	...
	const createSvg = (
		path: string,
		transform: Transform,
		uniformBoop: THREE.IUniform<any>,
		parent?: THREE.Mesh
	) => {
		...
		new SVGLoader()
			...
			.paths.flatMap((path) =>
				SVGLoader.createShapes(path).map((shape) => {
					...
					material.onBeforeCompile = boopShader(uniformBoop);
					...
					mesh.customDepthMaterial = new THREE.MeshDepthMaterial({
						...
						depthWrite: true,
						depthTest: true,
						onBeforeCompile: boopShader(uniformBoop)
					});
	...
	let svgs = new THREE.Group();
	svgs.userData = { boop: { value: $boop } };
	transforms.map((t) => svgs.add(createSvg(githubMarkWhite, t, svgs.userData.boop, box)));
...
<T.PerspectiveCamera makeDefault position={[0, -5, 10]} fov={45} {aspect} near={0.1} far={1000}>
	<OrbitControls target.y={1.5} enableZoom={false} enableDamping autoRotateSpeed={0.5} />
</T.PerspectiveCamera>

<T.DirectionalLight castShadow intensity={0.8} position={[12, 12, 10]} />
